...
export default function WhyMe() {
  const features = [
    ...
    //   why: "من ناحية التصميم, الميزات, و الوظائف سوف يكون موقعك مفصل خصيصا لك",
    //   svg: "final-product.svg"
    // },
    {
      title: "موعد التسليم",
      why: "واحد من اهم الامور في العمل بالنسبة لي هي المواعين والالتزام بيها تاكد انني لن افوته",
      svg: "dead-line.svg"
    },
  ]
  return (
    <section className="section-styling sm:h-[180vh] md:h-[108vh] lg:h-[124vh] xl:h-[101vh] ar">
      <div data-aos="fade-down" className="flex flex-col items-center mb-5">
        <h4 className="title">لماذا انا</h4>
      </div>
      <div className="w-[70%] h-[80vh] flex flex-col items-center justify-between mt-8 mx-auto">
          ...
          features.map(({ title, why, svg }, i) =>
            <div data-aos={`${i % 2 === 0 ? "fade-left" : "fade-right"}`} className={`flex items-center sm:flex-col sm:mt-8 md:mt-0 ${i % 2 === 0 ? "md:flex-row" : "md:flex-row-reverse"}`} key={i}>
              ...
              <div className={`${i % 2 === 0 ? "md:ml-2" : " md:mr-2"} sm:mt-3 md:mt-0`}>
                <h4 className="sm:text-[1.5rem] text-black font-bold">{title}</h4>
        ...
        <div className="flex flex-col items-center sm:mt-8 md:mt-0">
          <h5 className="text-[2.5rem] text-blue_color font-bold mb-5">مهتم؟</h5>
          <ul>
